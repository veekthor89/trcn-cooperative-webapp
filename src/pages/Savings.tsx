  ...
  const fetchSavingsData = async () => {
    try {
      ...
      const {
        data: accountsData,
        error: accountsError
      } = await supabase.from("accounts").select("*").eq("user_id", userId).eq("account_type", "savings");
      ...
      } = await supabase.from("special_contributions").select("*").eq("user_id", userId).in("application_status", ["active", "approved"]);
      ...
      <div className="space-y-6">
        ...
        {loading ? <div className="space-y-4">
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-4xl font-bold text-white">₦{totalSavings.toLocaleString('en-NG', {
            ...
            <div>
              ...
              {accounts.length === 0 ? <Card className="shadow-card">
                  ...
                  {accounts.map(account => <Card key={account.id} className="shadow-card">
                      <CardHeader>
                        <CardTitle className="flex items-center justify-between">
                          <span>Savings Account</span>
                          <span className={`text-sm px-2 py-1 rounded ${account.status === 'active' ? 'bg-secondary/10 text-secondary' : 'bg-muted'}`}>
                            {account.status}
                          </span>
                        </CardTitle>
                      </CardHeader>
                      <CardContent>
                        <p className="text-3xl font-bold mb-2">₦{Number(account.balance).toLocaleString('en-NG', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                  })}</p>
            ...
            <div>
              ...
              {contributions.length === 0 ? <Card className="shadow-card">
                  <CardContent className="py-12 text-center">
              ...
              return <Card key={contribution.id} className="shadow-card">
                        <CardHeader>
                        ...
                        <CardContent className="space-y-4">
                          ...
                          <div className="flex justify-between text-sm">
                            <div>
                              ...
                              <p className="font-semibold">₦{Number(contribution.total_contributed).toLocaleString('en-NG', {
                          minimumFractionDigits: 2
                        })}</p>
                            </div>
                            <div className="text-right">
                              <p className="text-muted-foreground">Expected</p>
                              <p className="font-semibold">₦{Number(contribution.total_expected).toLocaleString('en-NG', {
                          minimumFractionDigits: 2
                        })}</p>
                            </div>
                          </div>
                          {contribution.maturity_date && <p className="text-xs text-muted-foreground">
